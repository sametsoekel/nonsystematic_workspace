...
cross_validate <- function(train_data,target_data,folds,config,label_encoder = NULL,label_back_encoder = NULL){
    ...
    prm <- list(learning_rate = config$learning_rate,
                iterations = config$iter,
                loss_function = config$objective,
                min_data_in_leaf = config$min_n,
                depth = config$tree_depth,
                #auto_class_weights = config$auto_class_weights,
                verbose = 0,
                #custom_loss = config$loss_fun,
    ...
    for(i in folds_){
        ...
        prediction_to_threshold <- catboost.predict(cat_model,test_frame)
        #thresholded <- fold_threshold(y_probs = prediction_to_threshold,y_test = test_y)
        thresholded <- list()
        
        thresholded[['score']] <- rmse_vec(truth = test_y,estimate = prediction_to_threshold)
        
        msgformat_score <- 'Fold %s RMSE : %s'
        ...
        prediction <- catboost.predict(cat_model,catboost.load_pool(target_data$data))
